    ...
    <section class="tech-card live-workspace__summary">
      <div class="live-workspace__head">
        ...
        <div v-if="props.store.gpus.length" class="live-workspace__chip-row">
          <span class="live-chip">{{ props.store.dataSourceLabel.text }}</span>
          <span class="live-chip">{{ props.store.dataSourceStatus.gpu_count || props.store.gpus.length }} 卡</span>
        </div>
      </div>
      <div class="live-workspace__pulse">
        <div v-for="card in pulseCards" :key="card.label" class="live-workspace__pulse-card" :class="`live-workspace__pulse-card--${card.tone}`">
    ...
    <section class="tech-card live-workspace__gpu-surface">
      <div class="live-workspace__surface-head">
        <div>
          ...
          <div class="live-workspace__surface-note">把算力卡片集中在同一层，避免把治理摘要和实时硬件信息混在一起。</div>
        </div>
      </div>
      <div v-if="props.store.gpus.length" class="live-workspace__gpu-grid">
        <button v-for="gpu in props.store.gpus" :key="gpu.index" type="button" class="live-workspace__gpu-card" @click="router.push(`/gpu/${gpu.index}`)">
...
.live-workspace__summary,
.live-workspace__gpu-surface {
  padding: 22px;
}
.live-workspace__head,
