...
class FrontendUIStructureTests(unittest.TestCase):
    ...
    def test_governance_pages_mount_shared_capability_drawer(self):
        ...
        cluster_toolbar_text = (ROOT / "frontend/src/components/cluster/ClusterConsoleToolbar.vue").read_text(encoding="utf-8")
        ...
        self.assertIn("ClusterConsoleToolbar", cluster_text)
        self.assertIn("高级集群操作", cluster_toolbar_text)
    ...
    def test_cluster_console_route_and_components_exist(self):
        ...
        api_text = (ROOT / "frontend/src/services/api.js").read_text(encoding="utf-8")
        cluster_text = (ROOT / "frontend/src/views/ClusterJobs.vue").read_text(encoding="utf-8")
        allocation_text = (ROOT / "frontend/src/components/cluster/ClusterAllocationPanel.vue").read_text(encoding="utf-8")
        ...
        self.assertIn("listClusterNodes", api_text)
        self.assertIn("submitClusterJob", api_text)
        self.assertIn("listClusterQueues", api_text)
        self.assertIn("集群作业", governance_models)
        self.assertIn("allocation.release", cluster_text)
        self.assertIn("node.drain", cluster_text)
        self.assertIn("排空节点", allocation_text)
        self.assertIn("释放", allocation_text)
