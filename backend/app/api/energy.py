...
async def get_metrics(hours: float = Query(default=24.0, ge=0.5, le=168)):
    ...
    return await app_state.energy.get_energy_metrics(hours)
...
async def get_time_breakdown(hours: float = Query(default=24.0, ge=1, le=168)):
    """峰/谷/平能耗占比分析"""
    from app.main import app_state
    return await app_state.energy.get_time_period_breakdown(hours)
...
async def get_prediction(hours: int = Query(default=24, ge=1, le=72)):
    """功耗预测 + 置信带"""
    from app.main import app_state
    return await app_state.energy.get_power_prediction(hours)
...
async def get_carbon(hours: float = Query(default=24.0, ge=0.5, le=168)):
    """碳排放详情 + 等效树木"""
    from app.main import app_state
    return await app_state.energy.get_carbon_data(hours)
...
async def get_report(hours: float = Query(default=24.0, ge=1, le=168)):
    """全KPI增强报告"""
    from app.main import app_state
    return await app_state.energy.get_full_report(hours)
...
async def get_optimization_history(hours: float = Query(default=72.0, ge=1, le=720)):
    """优化历史记录"""
    from app.main import app_state
    return await app_state.store.get_optimization_history(hours)
...
async def export_report(
    ...
    """导出能耗分析报告"""
    from app.main import app_state
    content = await app_state.energy.generate_export_report(hours, format)
    if format == "html":
        return Response(
            content=content,
